...
def get_bench(bench_file, bench_name, pdb = None, download = True, kopis = False):
    bench_obj = []
    all_bench = {}
    ...
    with open(bench_file) as filin:
        for f in filin:
            ...
            all_bench[pdb_name]["size"] = get_size(pdb_file,pdb_name[-1])
    return all_bench


def get_size(pdb_file, chain):
    content = []
    model = False
...
def coverage_prot(predictions, max_length):
    max_length = int(max_length)
    full_prot = np.arange(1,max_length + 1)

    for i in range(len(predictions)):
        res1,res2 = predictions[i].get_delim()
        range_domain = np.arange(res1,res2+1)
        cov_prot = np.in1d(full_prot, range_domain).sum() / max_length * 100
        predictions[i].set_cov_gen(round(cov_prot,2))
...
def coverage_bench(predictions, bench):
    ...
    for i in range(len(predictions)):
        for j in range(len(bench)):
            bench_pred = bench[j].get_coverage(predictions[i])
            if bench_pred >= 85:
                predictions[i].set_bench(bench[j].get_delim())
                predictions[i].set_cov_bench(bench_pred)
        predictions[i].get_info()


def predict_full_bench(all_bench, bench_name):
    config = PUConfig() # Todo: load model's config from folder
    model_name = "../results/real_pad_1024_1048/mask_rcnn_real_pad_1024prot_1048_0024.h5"
    ...
    for pdb_code in all_bench:
        ...
        coverage_prot(predictions, last_pos)
        best = solutions(predictions, all_bench[pdb_code]["domain"])

        write_output(predictions, folder,name_pdb, bench_name, best)


def benchmark_get_correct(solution_list, bench_domain,bench_domain_size, all_correct, bench_dict):
        ...
        if len(all_sol_match) == length_ref: # only if same nb of domain
            ...
            for pred_delimitation in all_sol_match: # delim in one sol
                ...
                for ref in bench_domain:
                    if bench_dict[ref]:
                        uncov = (1 - (ref.get_uncov(obj_pred) / bench_domain_size)) * 100
                        coverage_ref_pred = ref.get_coverage(obj_pred)
                        coverage_pred_ref = obj_pred.get_coverage(ref)
...
def bench_bench_get_correct(solution_list, bench_domain, bench_domain_size, all_correct, bench_dict):
    ...
    for pred_delimitation in solution_list: # delim in one sol
        for ref in bench_domain:
            if bench_dict[ref]:
                uncov = (1 - (ref.get_uncov(pred_delimitation) / bench_domain_size)) * 100       
                coverage_ref_pred = ref.get_coverage(pred_delimitation)
                coverage_pred_ref = pred_delimitation.get_coverage(ref)
                # if coverage_ref_pred >= 85 and coverage_pred_ref >= 85:
                if uncov >= 85:
                    bench_dict[ref] = 0
                    correct += 1
...
def final_output(all_bench, bench_name, list_prot = False):
    nb_domain = 0
    nb_all_domain = 0
    nb_prot = 0
    all_correct = 0
    ...
    for prot in all_prot:   
        ...
        if solutions: # Get solution list
            ...
            all_correct_value, all_correct = benchmark_get_correct(solution_list, all_bench[prot]["domain"], all_bench[prot]["size"],  all_correct, bench_dict)
            print(len_dict,all_correct_value)
    ...
    return predict_to_csv(results_per_domain,bench_name, list_prot)



def predict_to_csv(results_per_domain, bench_name, list_prot = False):
...
def predict_all_benchmarks():
    ...
    for bench_name in b:
        ...
        #     continue
        if bench_name not in ["Jones_original","Islam90_original"]:
            continue
        ...
        #     predict_full_bench(all_bench, bench_name)
        prot_done, pourc = final_output(all_bench, bench_name, list_prot = True)
...
def compare_benchmarks(jones_islam_dict):
    jones_name = "Jones_original"

    islam_name = "Islam90_original"
...
def plot_results(results_dict):
    ...
    plot_hist(jones_label,jones_results, "../figures/Jones_1024_full_e24.png", "Jones_original")
    plot_hist(islam_label,islam_results, "../figures/Islam_1024_full_e24.png", "Islam90_original")
...
if __name__ == "__main__":
    ...
    jones_islam = predict_all_benchmarks()
    ...
    # bench = ["Islam90_DDAuth","Islam90_DDCath","Islam90_DDScop","Islam90_DP","Islam90_PDP","Islam90_original","SWORD_Islam90"]
    # for b in ["Islam90_DP"]  :
    #     bad = 0
    #     dom = 0
    #     ok_dom = 0
    ...
    #                 if dict_ref[ref]:
    #                     uncov = (1-(ref.get_uncov(pred) / all_bench_islam[prot]["size"])) * 100
    #                     print("Ref : {:15} Method : {:15} Cov  (1 - ({} / {})) * 100 = {:5.2f}".format(ref.name,pred.name,ref.get_uncov(pred),all_bench_islam[prot]["size"],float(uncov)))            
    ...
    #         # print()
    #         print("PPPPPPPPPPPPPPPPPPPPPPPPPPP",p)

    #     print(ok,"/",p, "=", ok /  p * 100 , "%", b)
        # # print_all_csv()
    
    """
    Bench :  Jones_original
